    ...
    open(component: Component, id?: number | any): NgbModalRef {
        ...
        if (id) {
            this.eventService.find(id).subscribe((event) => {
                event.startDateTime = this.datePipe
                    .transform(event.startDateTime, 'yyyy-MM-ddThh:mm');
                event.endDateTime = this.datePipe
                    .transform(event.endDateTime, 'yyyy-MM-ddThh:mm');
